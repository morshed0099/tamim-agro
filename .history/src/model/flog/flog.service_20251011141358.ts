...
const getAllFlock = async (payload: any) => {
  ...
  const whereCondition: Prisma.FlockWhereInput = { AND: adnCondition };

  const result = await prismaClient.flock.findMany({
...
const getFlockDetailsById = async (id: string) => {
  const retult = await prismaClient.flock.findUnique({
    where: {
      id,
    },
    include: {
